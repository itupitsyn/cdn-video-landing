...
export const QA: FC = () => {
  return (
    <section className="container pt-27.5">
      <h2 className="font-gotham-pro leading-subpup text-center text-[30px] tracking-tight">Вопросы и ответы</h2>

      <div className="flex flex-col gap-2 pt-7.5">
        {data.map((item, idx) => (
          <div
            key={item.id}
            className={cn('flex flex-col gap-4.75 px-5 py-6', idx > 0 && 'border-t border-solid border-white/20')}
          >
            <h3 className="font-golos-text leading-pup text-[18px]">{item.q}</h3>
            <p className="leading-extrasubpup text-white/70">{item.a}</p>
          </div>
        ))}
      </div>
    </section>
